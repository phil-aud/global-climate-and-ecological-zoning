...
│   ├── public/
│   └── package.json
├── Output/                # Published GIS products (Git LFS)
│   ├── raster/            # GeoTIFFs (GCZ, GEZ, HLZ levels)
│   └── vector/            # Zipped shapefiles
├── firebase.json
└── README.md
```
...
The published GIS products generated by this work are available under [`Output/`](Output/),
tracked with [Git LFS](https://git-lfs.com) (you need `git lfs install` to pull the actual files):

- `Output/raster/` — GeoTIFFs of the Global Climate Zones (GCZ), Global Ecological Zones (GEZ),
  and the three Holdridge Life Zone (HLZ) aggregation levels, for both the CRU TS 4.09
  (IPCC default) and TerraClimate climatologies over 1995–2024.
- `Output/vector/` — the same products as zipped shapefiles.
- `Output/raster/Legend_*.clr` and `GEZ_LEVEL_3symbology.lyrx` — colour maps / ArcGIS symbology.
- `Output/README_GCZ & GEZ.md` — class definitions, colour codes, and methodology notes.
